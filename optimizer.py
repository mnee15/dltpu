...
def create_optimizer(optimizer_name: str, params, lr, weight_decay=1e-5, momentum=0.9):
    if is_optimizer(optimizer_name):
        create_fn = optimizer_entrypoint(optimizer_name)
        if optimizer_name.startswith('Adam'):
            optimizer = create_fn(params=params, lr=lr, weight_decay=weight_decay)
        elif optimizer_name.startswith('SGD'):
            optimizer = create_fn(params=params, lr=lr, weight_decay=weight_decay, momentum=momentum)
